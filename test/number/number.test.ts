...
describe("Can manipulate numeric intervals", () => {
    ...
    test("Does an array contain this item? For infinite array only based on limits", () => {
    ...
    test("Concats two intervals. In non-overlapping cases, returns back both intervals", () => {
        ...
        expect(interval(2, 4).concat(interval(5, 7))[0].array()).toEqual([[2, 3, 4], [5, 6, 7]]);
    });
    test("Creates copy", () => {
        const i = interval(1, 100);
    ...
    test("Do two intervals overlap?", () => {
        ...
        expect(interval(2, 5).overlap(interval(1, 2))).toBe(false);
    });
    test("Are the interval borders container within", () => {
        expect(interval(2, 5).isInside(interval(5, 6))).toBe(false);
        expect(interval(2, 5).isInside(interval(3, 4))).toBe(true);
